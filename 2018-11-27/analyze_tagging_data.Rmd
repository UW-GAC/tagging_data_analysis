...
Finally, I subset the data to only include the seven cohort studies that received subcontract funding to complete tagging. The other studies, which were tagged by DCC staff members, have far fewer variables to tag and weren't easy to visualize on the same scale as the seven large cohort studies. I also added some study abbreviations for concise display.
```{r subset-studies}
...
phs_to_shortname <- names(funded_studies)
names(phs_to_shortname) <- as.character(funded_studies)
...
study_colors <- brewer.pal(length(levels(tagged_variables$study_shortname)), 'Set2')
