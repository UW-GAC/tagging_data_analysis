...
data_file <- '/projects/topmed/phenotype_tagging/internal_use_exported_data/2018-11-27_1552_dcc_tagging_data/tagged_variables.txt'
```
...
I read in the data using the `tidyverse` `readr` function. After examining the data, I set the data type for each column, including setting several categorical variables up to be factors with automatically deteckt factor levels.
...
Then I customized the factor levels for the `milestone`, `dcc_review_status`, and `study_response_status` columns. I confirmed the factor levels had been changed accurately by comparing bar plots of each variable before and after the level changes. The plots are hidden now.
```{r set-factor-levels, fig.show='hide', results='hold'}
...
Last time I said:
> Finally, I subset the data to only include the seven cohort studies that received subcontract funding to complete tagging. The other studies, which were tagged by DCC staff members, have far fewer variables to tag and weren't easy to visualize on the same scale as the seven large cohort studies. 

But now I want all of the studies included, so I'm not doing this subsetting. I still added some study abbreviations for concise display.
```{r subset-studies}
# # Save a subset of the data with just the funded studies.
...
```{r total-vars}
totals_fn <- '/projects/topmed/analysts/emeryl/tagging_data_analysis/2019-04-16/total_traits.txt'
